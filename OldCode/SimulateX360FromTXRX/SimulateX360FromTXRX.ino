...
//AT+UART=57600,1,0  For HC05
//const long m_baudrateBT=115200;
//const long m_baudrateBT=57600;
const long m_baudrateBT=9600;
//Note: I tried 9600 is too slow and 57 115 lose to much package.
// Can I do something about it... Not realy except rerwork the code ot use 1010 byte instead of string. To limite the byte to read.
...
void ExecuteWaitingCommand(char* command) {
  ...
  if (cmd.indexofrelative('%') > -1) {
    ...
    if (cmd.startwith('j', 'l', 'h', '%') || cmd.startwith('j', 'l', 'x', '%')  ) {
      m_jlh = (value+1.0)/2.0;
      if (m_debugModeSerial) Log(cmd, '!', "\njlh " + String(m_jlh) + " " + String(m_jlv));
    }
    if (cmd.startwith('j', 'l', 'v', '%')|| cmd.startwith('j', 'l', 'y', '%') ) {
      m_jlv = (value+1.0)/2.0;
      if (m_debugModeSerial) Log(cmd, '!', "\njlv " + String(m_jlh) + " " + String(m_jlv));
    }
    if (cmd.startwith('j', 'r', 'h', '%')|| cmd.startwith('j', 'r', 'x', '%') ) {
      m_jrh = (value+1.0)/2.0;
      if (m_debugModeSerial) Log(cmd, '!', "\njrh " + String(m_jrh) + " " + String(m_jrv));
    }
    if (cmd.startwith('j', 'r', 'v', '%')|| cmd.startwith('j', 'r', 'y', '%') ) {
      m_jrv = (value+1.0)/2.0;
